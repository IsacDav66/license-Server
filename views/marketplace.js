const renderMarketplace = (plugins) => {
    ...
    <head>
        ...
        <meta property="og:title" content="StunBot | Marketplace">
        <meta property="og:description" content="Explora nuestras extensiones premium y potencia tu bot.">
        <meta property="og:image" content="https://davcenter.servequake.com/stunbot/logo.png">
        <meta property="og:url" content="https://davcenter.servequake.com/stunbot/marketplace">
        ...
        <style>
            ...
            /* --- CONTROLES --- */
            .controls-wrapper { display: flex; gap: 15px; margin-bottom: 25px; background: rgba(255,255,255,0.02); padding: 12px; border-radius: 16px; border: 1px solid var(--border); align-items: center; }
            .search-box { flex: 1; position: relative; }
            .search-box i { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: var(--primary); }
            .search-box input { width: 100%; padding: 12px 15px 12px 45px; border-radius: 10px; border: 1px solid var(--border); background: #0a0f1a; color: white; outline: none; transition: 0.3s; box-sizing: border-box; }
            .sort-box select { padding: 12px; border-radius: 10px; border: 1px solid var(--border); background: #0a0f1a; color: white; cursor: pointer; outline: none; }

            /* --- FILTROS --- */
            .filter-wrapper { margin-bottom: 40px; text-align: center; }
            .filter-bar { display: flex; justify-content: center; gap: 8px; flex-wrap: wrap; overflow: hidden; max-height: 95px; transition: max-height 0.4s ease; }
            ...
            .show-more-cats { display: inline-block; background: none; border: none; color: var(--primary); font-weight: 800; font-size: 0.7rem; cursor: pointer; margin-top: 15px; text-transform: uppercase; }
            ...
            /* --- MODAL DE LICENCIA (AQUÍ ESTÁ LO NUEVO) --- */
            .license-modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 3000; justify-content: center; align-items: center; backdrop-filter: blur(5px); }
            .license-card { background: var(--card); padding: 40px; border-radius: 30px; border: 1px solid var(--border); text-align: center; width: 90%; max-width: 400px; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
            ...
            @media (max-width: 768px) { .controls-wrapper { flex-direction: column; } .plugin-grid { grid-template-columns: 1fr; } .filter-bar { max-height: 95px; } .show-more-cats { display: inline-block; } }
        </style>
    </head>
    <body>
        ...
        <div class="container">
            ...
            <div class="filter-wrapper">
                <div class="filter-bar" id="filterBar">
                    ${categories.map(cat => `
                        <button class="filter-btn ${cat === 'Todos' ? 'active' : ''}" onclick="setCategory('${cat}', this)">
                            ${cat}
                        </button>
                    `).join('')}
                </div>
        ...
        <!-- MODAL DE LICENCIA (Aparece al comprar) -->
        <div id="licenseModal" class="license-modal" onclick="if(event.target == this) this.style.display='none'">
            <div class="license-card">
                <i class="fas fa-shield-halved" style="font-size:2.5rem; color:var(--primary); margin-bottom:15px;"></i>
                <h2 style="margin: 0; color: white;">Licencia Requerida</h2>
                <p style="color: #94a3b8; font-size: 0.85rem; margin: 15px 0 25px;">Para adquirir extensiones debes tener el bot base activado.</p>
                <input type="text" id="m_key" style="width:100%; padding:15px; border-radius:12px; background:#000; border:1px solid #1e293b; color:white; text-align:center; margin-bottom: 20px;" placeholder="STUNBOT-XXXX-XXXX">
                <button style="width:100%; background:var(--primary); padding:15px; border:none; border-radius:12px; font-weight:800; cursor:pointer;" onclick="validateAndBuy()">VALIDAR Y COMPRAR</button>
            </div>
        </div>
        ...
        <script>
            ...
            function toggleCategories() {
                ...
                document.getElementById('toggleCatsBtn').innerHTML = isExpanded ? 'Ver menos <i class="fas fa-chevron-up"></i>' : 'Ver todas las categorías <i class="fas fa-chevron-down"></i>';
            }
            ...
            function renderGrid(list) {
                ...
                grid.innerHTML = list.map(p => {
                    ...
                    const reqHtml = (p.requirements && p.requirements.length > 0) ? 
                        '<div class="req-info"><i class="fas fa-circle-question"></i><div class="req-tooltip"><b>Requerimientos:</b>' + p.requirements.map(r => '<li>'+r+'</li>').join('') + '</div></div>' : '';
                    ...
                    return \`
                        <div class="plugin-card">
                            ...
                            <div style="font-family:monospace; font-weight:800; font-size:1.1rem; color: white;">$\${p.price} USD</div>
                            <div class="btn-group">
                                <button class="btn-preview" onclick="openPreview(\${origIndex})">VISTA PREVIA</button>
                                <button class="btn-buy" onclick="requestPurchase(\${p.tebex_id})">ADQUIRIR</button>
                            </div>
                        </div>
                    \`;
                }).join('');
            ...
            function requestPurchase(pkgId) {
                selectedPackageId = pkgId;
                document.getElementById('licenseModal').style.display = 'flex';
            }

            async function validateAndBuy() {
                const key = document.getElementById('m_key').value;
                if(!key) return alert('Ingresa tu licencia');
                
                const res = await fetch('/stunbot/verify', { 
                    method: 'POST', 
                ...
                if(data.valid) {
                    ...
                    Tebex.checkout.openPackage(selectedPackageId);
                } else {
